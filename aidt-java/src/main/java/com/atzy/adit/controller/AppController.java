...
import com.atzy.adit.model.dto.app.AppAddRequest;
import com.atzy.adit.model.dto.app.AppEditRequest;
import com.atzy.adit.model.dto.app.AppQueryRequest;
import com.atzy.adit.model.dto.app.AppUpdateRequest;
...
public class AppController {
    ...
    public BaseResponse<Boolean> reviewApp(@RequestBody ReviewRequest reviewRequest, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(id == null || enumByValue == null, ErrorCode.PARAMS_ERROR);
        //查询是否有相应的应用
        App Oldapp = appService.getById(id);
        ThrowUtils.throwIf(Oldapp == null, ErrorCode.NOT_FOUND_ERROR);
        //已经是该状态
        if (Oldapp.getReviewStatus().equals(reviewStatus)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "已经是该状态");
        }
        //更新审核状态
        User loginUser = userService.getLoginUser(request);
        App app = new App();
